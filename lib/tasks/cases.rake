namespace :db do
  ...
  task :fix_case => :environment do
    ...
    when 'paulina bravo' 
      ...
      first = nil
      user.services.ordered_by_created_at.each_with_index do |service, index|
        if service.datetime.in_time_zone('Etc/GMT+6').weekday == 'thursday'
          puts "found service service id #{service.id} recurrence id #{service.recurrence_id} service type #{service.service_type.name} #{service.tz_aware_datetime.weekday} #{service.tz_aware_datetime}"
          # service.recurrence = r
