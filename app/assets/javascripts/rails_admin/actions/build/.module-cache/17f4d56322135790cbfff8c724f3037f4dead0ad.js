  ...
  componentDidMount: function () {
    $.ajax({
      ...
      _.each( data, function( schedule ){
        ...
        var start_hour = 0;
        console.group('Schedule Hours', schedule.id, schedule_hours );
        ...
        for(; start_hour < schedule_hours ; start_hour++ ){
          ...
          if( schedule.service_type_id === 1){
            $('.'+class_name).html( schedule.user.full_name ).find('recurrent').addClass('active');
          }
          if( schedule.service_type_id === 2){
            $('.'+class_name).html( schedule.user.full_name ).find('one_time').addClass('active');
          }
        }
...
var ScheduleCell = React.createClass({displayName: "ScheduleCell",
  render: function() {
    return (React.createElement("td", {className: this.props.class_name}, 
      React.createElement("div", {class: "recurrent"}), 
      React.createElement("div", {class: "one_time"}), 
      React.createElement("div", {class: "padding"}), 
      React.createElement("div", {class: "text"}, this.props.text)));
  }
});
