  ...
  componentDidMount: function () {
    $.ajax({
      ...
      _.each( data, function( schedule ){
        ...
        var schedule_hours = parseInt(schedule.estimated_hours).toFixed();
        var start_hour = 0;
        ...
        for(; start_hour < schedule_hours ; start_hour++ ){
          ...
          console.log( start_hour, schedule_day.format() );
          ...
          $('.'+class_name).addClass('schedule_padding').html( schedule.user.full_name );
        }
        console.groupEnd();
      });
    });
