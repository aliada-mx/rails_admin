...
var SchedulesApp = React.createClass({
  ...
  render: function() {
    ...
    return ( 
      <div>
        <SchedulesList slist={this.state.schedules} />
      </div>
    );
  }
});
...
var SchedulesList = React.createClass({
  render: function() {
    ...
    for(; current_hour <= last_hour; current_hour++){
      ...
      schedules.push( <ScheduleRow key={row_key} hour={current_hour} /> );
    }
    return ( 
      <div>
        <table className='table'>
          <thead><tr><th></th><th>Domingo</th><th>Lunes</th><th>Martes</th><th>Miércoles</th><th>Jueves</th><th>Viernes</th><th>Sábado</th></tr></thead>
      ...
      <tr>
        {
          week_table_array.map( function( day ) {
            ...
            return <ScheduleCell key={ class_name } text={ row_text } class_name={ class_name } />;
          })
        }
      </tr>
    );
